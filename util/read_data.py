...
def read_relatedness_dataset(direc, vocab, debug=False):
  ...
  dataset['id2boundary']={}
  for i in range(len(dataset['boundary'])-1):
    bound = dataset['boundary'][i:i+2]
    posbound = [bound[0], bound[0] + dataset['numrels'][i]]
    negbound = [bound[0] + dataset['numrels'][i], bound[1]]
    for j in range(bound[0], bound[1]):
      dataset['id2boundary'][j] = [bound, posbound, negbound]
  return dataset
