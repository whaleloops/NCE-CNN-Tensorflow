...
def train(train_dataset, dev_dataset, test_dataset, vecs, iter_num = 10000):
    ...
    model = model_name(vecs, dim=FLAGS.dim, seq_length=FLAGS.max_length)
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_string('model', 'rnn-pairwise', 'the version of model to be used')
    #tf.app.flags.DEFINE_string('num_pairs', 8, 'number of negative samples for each pos sample')
    tf.app.flags.DEFINE_string('dim', 100, 'dimension of hidden layers')
    tf.app.flags.DEFINE_string('lr', 1e-3, 'learning rate')
    tf.app.flags.DEFINE_string('batch_size', 64, 'mini-batch size')
