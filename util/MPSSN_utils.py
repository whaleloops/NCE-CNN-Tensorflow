...
def compute_euclidean_distance(x, y):
    with tf.name_scope('euclidean_distance'):
        d = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(x, y)), axis=1) + 1e-6)
        return d
...
def comU1(x, y):
    #t1 = compute_euclidean_distance(x, y)
    t1 = compute_cosine_distance(x, y)
...
def comU2(x, y):
    #result = [compute_euclidean_distance(x, y), compute_euclidean_distance(x, y)]
